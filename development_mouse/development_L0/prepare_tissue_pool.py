...
class PrepareTissuePool(luigi.Task):
	...
	def requires(self) -> List[luigi.Task]:
		samples = cg.PoolSpec(dm.paths().poolspec).samples_for_tissue(self.tissue)
	...
	def run(self) -> None:
		...
		metadata_file = os.path.join(dm.paths().samples, "metadata", "metadata.xlsx")
		...
		with self.output().temporary_path() as out_file:
			...
			vgpath = os.path.join(dm.paths().build, "genes.txt")
