...
class Cytograph2:
	...
	def fit(self, ds: loompy.LoomConnection) -> None:
		...
		if "Batch" in ds.ca and "Replicate" in ds.ca:
			technical = cg.identify_technical_factors.identify_technical_factors(theta, ds.ca.Batch, ds.ca.Replicate)
		...
		nn = NNDescent(data=theta, metric=cg.metrics.jensen_shannon_distance)
		...
		umap_embedder = UMAP(metric=cg.metrics.jensen_shannon_distance, spread=2, repulsion_strength=2, n_neighbors=self.k, n_components=2)
		umap = umap_embedder.fit_transform(theta)
		ds.ca.UMAP = umap

		logging.info(f"3D UMAP embedding from latent space")
		umap3d = UMAP(metric=cg.metrics.jensen_shannon_distance, spread=2, repulsion_strength=2, n_neighbors=self.k, n_components=3).fit_transform(theta)
		ds.ca.UMAP3D = umap3d
		...
		if "spliced" in ds.layers:
			...
			gamma, _ = cg.velocity_inference.fit_gamma(s, u)
